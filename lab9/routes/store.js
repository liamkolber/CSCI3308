    ...
    */
    req.getConnection(function(error, conn) {
        conn.query( 'SELECT * FROM store WHERE id = '+ req.params.id , function(err, rows, fields) {
            ...
            console.log("Twest" + JSON.stringify(rows));
            // if (rows.length <= 0) {
...
// EDIT ITEM POST ACTION - Update the item, actual update happenshere
app.put('/edit/(:id)', function(req, res, next) {
    ...
    if (!errors) { //No errors were found.  Passed Validation!
        ...
        req.getConnection(function(error, conn) {
            conn.query('UPDATE store SET ? WHERE id = ' + req.params.id, item,
                function(err, result) {
	...
	req.getConnection(function(error, conn) {
		conn.query('DELETE FROM store WHERE id='+req.params.id, function(err, result) {
			if (err) {
				...
				req.flash('success', /*Success message goes
