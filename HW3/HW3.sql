...
-- 1
SELECT LastName, FirstName FROM nwEmployees
    WHERE (Country <> 'USA') AND (HireDate < '2013-03-11 00:00:00')
...
SELECT ProductName, UnitPrice FROM nwProducts
    WHERE UnitPrice >= (SELECT MAX(a.UnitPrice) FROM (SELECT UnitPrice FROM nwProducts) a);


-- 4.
SELECT  ProductID, ProductName, UnitsInStock * UnitPrice AS 'Total Inventory Value' FROM nwProducts
    WHERE UnitsInStock * UnitPrice > 2000
    ORDER BY UnitsInStock * UnitPrice DESC;

-- 5
SELECT ShipCountry, COUNT(*) AS OrderCount FROM nwOrders
    WHERE (ShipCountry <> 'USA') AND (ShippedDate > '2013-09-01 00:00:00') AND (ShippedDate < '2013-10-01 00:00:00')
...
SELECT nwCustomers.CustomerID, nwCustomers.CompanyName FROM nwCustomers
    ...
    GROUP BY nwOrders.CustomerID HAVING COUNT(*) > 20;


-- 7
SELECT SupplierID, SUM(UnitPrice * UnitsInStock) AS InventoryValue FROM nwProducts
...
SELECT nwSuppliers.CompanyName,nwProducts.ProductName,nwProducts.UnitPrice FROM nwSuppliers
    JOIN nwProducts ON nwSuppliers.SupplierID = nwProducts.SupplierID
    WHERE Country = 'USA'
    ORDER BY nwProducts.UnitPrice DESC;


-- 9
SELECT E.LastName,E.FirstName,E.Title,E.Extension,COUNT(O.EmployeeID) OrderCount FROM nwEmployees E
...
-- 11
SELECT S.CompanyName,S.ContactName,C.CategoryName,C.Description,P.ProductName,P.UnitsOnOrder FROM nwProducts P
...
-- 12
SELECT nwProducts.ProductName,nwSuppliers.CompanyName,nwSuppliers.Country,nwProducts.UnitsInStock FROM nwProducts
...
INSERT INTO Top_Items(ItemID, ItemCode, ItemName,InventoryDate, ItemQuantity,ItemPrice, SupplierID)
    SELECT ProductID, CategoryID, ProductName, curdate(), UnitsInStock, UnitPrice, SupplierID FROM nwProducts
